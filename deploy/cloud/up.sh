...
# 池器域名 → IPv4 覆盖：只动 @ 后的 host 段（[^@/:]+ 不越过凭据/端口/路径），
# 独立函数便于离线自测（与生产路径同一份 sed 逻辑）。
apply_db_override() {
  printf '%s' "$1" | sed -E "s|@[^@/:]+\\.pooler\\.supabase\\.com|@${BOK_SUPABASE_IP:-52.77.146.31}|"
}
...
# ── 幂等覆盖：pooler 域名 → IPv4（已是 IP 则原样通过）
_db_after="$(apply_db_override "$DATABASE_URL")"
