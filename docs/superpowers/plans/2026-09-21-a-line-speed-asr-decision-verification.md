...
## 40. 用户五问的实测答复（2026-09-21）：max_tokens / 回复质量 / 意图识别 / 垫话 / 纪要质量

用户连问五条：*max_tokens 会不会太短？回复质量准确度怎么样？意图识别呢？反应这么快都不要垫音了？
知识库沉淀、通话纪要的质量呢？* 本节的**全部数字都来自真实库 + 真实端点**，没有一条靠推。

### 40.1 `max_tokens=160` 会不会太短——**不缺，且它只砍到过 1 次**

真库 A 线 `gen='llm'` 的 **725 条真回复**（用真 4B tokenizer 计数）：

| p50 | p75 | p90 | p95 | p99 | max | >160 |
|---|---|---|---|---|---|---|
| 19 | 29 | 42 | 50 | 83 | **159** | **0 条** |

`>120` 只有 5 条（0.7%）。而 **max=159 那条是被砍断的**：它的结尾是
`好，唔該晒你今日嘅`，而同长度的另外两条完整收在 `…多謝你嘅配合，拜拜！`——
即 **725 条里 1 条（0.14%）丢了告别语**。

结论：`max_tokens` 是**天花板不是目标**（p50 19 条的回复在 160 与 512 下代价相同，
模型到 EOS 就停），所以它**调大几乎免费、调小也不省**。要留余量只值 +32~64，
别为这 0.14% 大改。

### 40.2 回复质量：云端赢在**社交层**，本机 4B 输得很难看

`scripts/probe_reply_parity.py`——**12 条真实客户话 + 真实 system 前缀与尾块**，三腿并排。
两条最刺眼的（真记录，非构造）：

| 客户 | 本机 4B | deepseek-flash | v4-pro |
|---|---|---|---|
| 「同你讲嘢真系好难。**頂**。」（发火） | 「你好，**恭喜**你的案件已進入處理階段。」 | 「明白你**嬲**。」 | 「我講快咗少少。」 |
| 「**聽唔切**。你头先讲咩嚟？」 | 跳到第 4 步要 WhatsApp | 「我講慢啲。」+ 续讲 | 「我**講多次**：…單號尾號七八九零唔見咗。」 |

另有「我想先問下賠幾多？」（本机 4B 完全无视问题、照念第 1 步稿；云端答「賠償要核對咗先講到」
再带回）/「我要正式投訴」（本机 4B 不认投诉；云端「你嘅投訴我哋會認真跟進」）。
...
|---|---|---|
| 本机 4B | **438ms** | 936ms |
| DS-flash | 550ms | **797ms** |
| DS-v4pro | 771ms | 1045ms |

**本机 4B 的 p50 比云端还快**——§38 里「本地 p50 1010ms」是**真通话争用下**的读数，
不是空闲+前缀命中档。所以：**换云买的是质量与「不占本地 GPU」，不是延迟**。
公平起见：本探针绕过了规则层/罐头/直念步，测的是**残留的自由 LLM 轮**；
那些社交层的坏回复在真通话里有一部分会被快路截走。

### 40.3 意图识别：云端更贴 prompt 自己的 route 定义，本机 9B 爱误报 transfer_human

`scripts/probe_judge_parity.py`（10 条真实客户话，route 模式，预算 24）：

| 客户 | 本机 9B | DS-flash | DS-v4pro | 判读 |
|---|---|---|---|---|
| 「加你WhatsApp得唔得？」×2 | **transfer_human** | capture_contact | capture_contact | 云端对（问加 WhatsApp≠要真人；误报转人工有真实副作用） |
| 「我要正式投诉你哋」 | transfer_human | **register_followup** | register_followup | 云端对（prompt 明写 register_followup 含「投诉要求处理」） |
| 「我上一单就是淘宝买的」 | capture_contact | keep | keep | 云端对 |
| 「帮我跟进下个件」 | register_followup 0.8 | 0.9 | 0.9 | 三方一致 ✓ |
| 「聽唔切」/「我唔系好明你讲乜」 | degrade_question | degrade_question | degrade_question | 三方一致 ✓ |

一致率：`flash vs v4pro 6/10`、`9B vs flash 5/10`、`9B vs v4pro 4/10`。

**没有硬造准确率**：真库没有 route 人工标注（route 一命中就被消费掉），能客观给的只有一致率与分歧清单。
**且必须提醒**：§39 修完 conf 截断后 conf 普遍落在 **0.7~0.9**，于是
`register_followup` 会真的开始建单——而 `call_followups` 表**至今 0 行**，
说明这条链路**从未在真实通话里跑过**。修好的是「它本该能跑」，**不是「它已经验证过」**；
上线前建议跑一趟 soak（`scripts/probe_offscript_soak.py`）看建单与确认语的真实表现。

### 40.4 垫话还要不要——**要**，云端改变不了这件事

真库口径（本机 4B 生产档）：**553 个垫话轮**，**251/1164 通（21.6%）通话至少垫过一次**；
真 LLM 轮 `latency_ms` **均值 1201ms、最大 9742ms**。而垫话的触发线是 `BOK_FILLER_DELAY_MS=500ms`
（且它等的是**首段音频**，不是首 token）。

云端首 token 实测 p50 550ms，**再加 TTS 首包**才轮到出声——仍高于 500ms。
所以：①换云不会让垫话消失；②真通话的 p95 尾巴（最长 9.7s）正是垫话存在的理由；
③真要不要动，该动的是**门槛**而不是「拆掉兜底」。

### 40.5 纪要/沉淀质量：**本机 9B 在长通话上 JSON 基本必坏**（本轮新增发现）

`scripts/probe_settle_parity.py`——真实通话转写，`Summarizer._via_llm`，同一份 system 与 payload：

| 腿 | 短通话（转写 111 字） | 长通话（612 / 769 / 799 / 1162 / 1217 字） |
|---|---|---|
| 本机 **4B** @1235 | 4/4 | **6/6** |
| 本机 **9B** @1237（**当前 settle 专线**） | 4/4 | **0/14**（另一次批次 1/16） |
| DS-flash（思考开） | — | 3/3 |
| DS-v4pro（思考开） | — | 3/3（**修超时前是 0/5**） |

9B 的坏法不是截断：`finish_reason=stop`、只用了 312/512 token、**JSON 结构自身有病**
（`Expecting ',' delimiter`；brace 扫描也永不闭合）。`_parse` 于是静默退 `_fallback`
→ **那一通的 summary/new_topics/insight 全丢**（`new_topics` 是沉淀进知识库的原料）。
长度敏感性已双向验证：**111 字 4/4 正常、612 字以上 0/14**。
按真库分布，转写 ≥400 字的通话约 **80/1113（7%）**，即**约 7% 的话（且是最有信息量的那批）丢沉淀**。

云端两条腿不光不坏，还**多给了质检视角**——flash 在几通上主动标出
「索要微信/截图/第三方银联办理与正规理赔渠道不符，存在较高诈骗或信息泄露风险」，
这正好是该业务最该被沉淀的东西。

**顺带修掉两个「换云结构上走不通」的点**（都在 `summarize.py`）：
① `_via_llm` 以前**不带 Authorization**，云端点一律 401（我第一轮实测两腿全 401）；
② 思考档一次要 9-14s，而 `Summarizer` 缺省超时 15s → v4-pro **5/5 ReadTimeout**，
即「光抬预算不抬超时」跑不通。两条已修 + 加测试，并给 `_parse` 补上**可见日志**
（这个盲区正是「settle 模型换了以后沉淀成片丢」能藏住的原因）。

### 40.6 落到行动

1. **`max_tokens` 不动**（或 +32 留余量）；那条被砍的告别语不值得为它改架构。
2. **垫话保留**；要调只调门槛，别拆兜底。
3. **主回复换云的收益重估**：质量（尤其社交层）+ 释放本地 GPU，**不是延迟**——p50 本机更快。
4. **settle 专线要么换回 4B、要么走云**（云端需 §40.5 那两处修复 + 更长超时）；
   当前 9B 配置在长通话上等于没有沉淀。**这是本轮最该马上决定的一条。**
5. route 建单链路**修好了但没验证过**，上线前跑 soak。

---

## 41. 用户三问的回执（2026-09-21 续）：为什么会静默 / 本地能不能救 / 真实测试的边界

用户追问三件事：*为什么会静默丢数据？主 LLM 上云对话质量怎么样？沉淀纪要即时性要求不高，
可以考虑本地处理对吧？——我觉得应该都得真实测试。*

### 41.1 「为什么会静默丢数据」——三层，每一层都把失败伪装成成功

链路是 `Summarizer.build → _via_llm → _parse → _fallback`，而**三层都吞掉失败**：

1. **`_parse` 解析失败 → `_fallback`，不打任何日志**（本次已修）。
2. **`_fallback` 不是「无结果」，是一个**看起来正常**的桩**：
   `"本场共 {n} 轮。主要内容：{前两轮各 60 字}"`——**非空**。
3. **下游全部按「成功」记账**（`main.py` settle 段）：
   `result["summary"] = summ["summary"]`（拿到的是桩），
   `if result.get("new_topics"):` —— 空列表直接跳过，
   `_audit("settle.create", detail={"has_summary": bool(result["summary"])})` —— **桩让 has_summary=True**。

于是最终形态是：**settlements 有一条记录、summary 字段有字、审计写 success、topics 表安静地什么都没加**
——而「这通没挖出话题」与「模型坏了没产出」在数据上**完全同形**。
这就是它能藏住的全部原因：不是没人看，是**看什么都正常**。

> 修法（已落地）：`_parse` 两条失败路径都打日志（内容长度 + 头部 / 解析异常）。

### 41.2 本地能救回来——**而且救回来了**（推翻 §40.6 第 4 条的结论）

把实测的坏样本逐字打出来看，**不是内容坏，是只漏了最外层一个 `}`**（6/6 同一形态）：
...
`finish_reason=stop`、312/512 token——**不是截断**，是模型自己漏收尾。
而这种形状是**可以确定性补回来的**：新增纯函数模块 `bok_voice_core/json_repair.py`
（`close_unclosed_json`/`loads_lenient`），只做一件事：字符串感知地扫括号栈，
把欠着的闭括号按栈序补到末尾；**错位闭括号 / 未终止字符串 / 本来就平衡但解析失败一律返 `None`**
（那是内容坏了，补不出来，宁可不救——**修坏的 JSON 会把错答案沉淀进知识库，比丢掉更糟**）。

**实测收益（同一批真实转写，直打本机 9B）**：严格解析 **0/10** → 经补齐 **10/10**
（每条都拿回 summary + 2-3 条 topics + insight）。

所以用户那个判断是对的：**沉淀/纪要即时性要求不高，本地处理即可**——
它本来就该留在本地，**只是需要把「漏收尾」这类可确定性修复的坏法救回来**，
而不是为了绕过它去换模型或上云。§40.6 第 4 条「settle 换 4B 或走云」**据此作废**。

### 41.3 真实测试的边界（如实记，含没做到的原因）

| 面 | 测了什么 | 到什么层级 |
|---|---|---|
| settle JSON 坏法 | 真实转写 + 真 9B 端点 + 真 prompt，16 次采样定坏率、10 次验证补齐 | **真组件级**，不是构造样例 |
| main.py 的静默三件套 | 读真代码路径 + 真库审计字段语义 | 代码级证据 |
| 长通话通篇归属 | 想用「通话结束 10 分钟内该对象有没有新 topic」做生产侧佐证 | **不成立，已放弃**：`object_topics` 只挂 object 不挂 call，同一 object 被多通复用，时间近邻归属假阳/假阴都有——**没有把它当证据** |
| **真通话端到端（我的改动生效后）** | **没做** | 见下 |

**没做真通话端到端的原因（不是省事）**：本机此刻跑着的栈
（CP pid 2016 / A 线 worker pid 20880 / monitor pid 20643）**cwd 是主树
`/Users/halo/Documents/bok/voice-assistant`**，而主树 HEAD 停在 `726cd2c`（#139）且**工作区是脏的**
（agent.py / bok.py 等 5 个文件有未提交改动）；`git worktree list` 显示**另有个 4 个会话 worktree 并存**
（最近一个是 16:06 开的）。也就是说：**那套栈不是我的、也不是 main 的，而可能有别的会话正在写**。
要让它加载我的补丁必须重启 CP —— 那会打断别人在跑的东西。
按本仓「不碰别人栈」的纪律，**这一步我不擅自做**。

要真跑端到端，二选一（都需你点头）：
1. 你确认现在没有别的会话在用那套栈 → 我按 §36.6 的同款姿势接管（停 monitor + A 线 worker，
   以本树代码与目标 env 起 worker，跑完交还）；
2. 或者等我的补丁进 main、你那边自然重启栈之后，我用真通话（测试对象）跑一发
   「挂断 → settle → 查 `object_topics` 是否真写进去」，把 41.2 的组件级结论升到端到端。

### 41.4 主 LLM 上云的对话质量——目前是**离线但真端点**的结论，端到端待上条通道

已有的（§40.2，12 条真实客户话 × 三腿，真前缀尾块）：云端在**社交层**明显更好
（发火轮 4B 回「恭喜」vs flash 回「明白你嬲」；「聽唔切」云端会重讲、4B 跳步），
且首字 p50 **本机 4B 438ms 反而更快**（换云买质量与不占 GPU，不买延迟）。
**这仍是回放，不是真通话**——理由同上（换 provider 必须重启 worker）。
端到端要做的话，判据应是**真通话录音 + 逐轮人工比对**，而不是再看一遍回放。

---

## 42. 真实通话延迟测试（2026-09-21，用户：「垫音是遮羞布，先用真实测试一下」）

用户拍板：**延迟第一、质量第二**；主 LLM 上云**不必**（本机 4B p50 更快）；
垫音=遮羞布（让客户觉得没等那么久）。要求先真测。

### 42.1 测法：从**外部**打时间戳，不插桩、不重启共享栈

新探针 `scripts/probe_reply_latency.py`：作为客户加入房间，推真实 TTS 客户话音，
按 100ms 采样 agent 两条音轨（主音轨 + 垫话 out-of-band `background_audio`），
用能量口径切「出声段/静音段」，并把本轮 `agent.log` 增量里的
`eou delay` / `BOK_FILLER fired` / `watchdog` / `LLM_TTFT_MS` / `tts ttfb` 一并归因。
对象走 `E2E-` 前缀族 → 心跳豁免（否则 nudge 污染测量窗）。

**为什么不去重启那套栈**：它 cwd 是**主树脏工作区**（HEAD 停 #139、5 个文件未提交），
且 `git worktree list` 显示**另有 4 个会话 worktree 并存**——重启即打断别人。
外部打点**不需要**动 agent 一行代码。

### 42.2 健康通话的真实预算（6 轮，`call-54ed586a`）

| 轮 | eou(停嘴→成轮) | LLM TTFT | TTS ttfb | 外部实测首声 | 外部实测真回复 | 垫话 |
|---|---|---|---|---|---|---|
| 1 | 486ms | 678ms | **1945ms** | 1500ms | 2600ms | ✓ |
| 2 | 747ms | 779ms | 38ms | 2700ms | 2700ms | — |
| 3 | 696ms | 714ms | **1071ms** | 1800ms | 4000ms | ✓ |
| 4 | 748ms | 800ms | 36ms | 2800ms | 2800ms | — |
| 5 | 1022ms | 711ms | **1398ms** | 2100ms | 4400ms | ✓ |
| 6 | 777ms | 706ms | 40ms | 3000ms | 3000ms | — |

汇总：**遮羞布首声 p50 2700ms / 真回复首声 p50 3000ms**（max 4400ms）；
垫话开火时**裸洞只有 300-600ms**（遮羞布基本盖得住）。
LLM 很稳：**TTFT 678-800ms、tps 15-24、`cached` 逐轮 1693→2244 递增**（KV 前缀缓存正常工作）。

**三腿都不是瓶颈**：LLM 0.7s + TTS 首包 0.04s + eou 0.75s ≈ **1.5s** 的真实通路。

### 42.3 反直觉的那一条：**遮羞布把真回复推迟了 1-2 秒**（自己掐自己）

看 ttfb 那一列：**36-40ms 与 1071/1398/1945ms 两态，而高值那三轮正好是垫话开火的三轮**
（1/3/5）。原因在设计里：`_RelaySynthesizeStream.hold_if_playing()` 把回复**首帧**扣住，
等垫话播完 + `BOK_FILLER_GAP_MS`(300ms) 才放——垫话 1.6-2.3s + gap，就是那 1.9s 的 ttfb。

也就是说：**回复文本 ~1.2s 就绪，音频被硬按到 ~2.6s 才出声**。
遮羞布买到的是「1.5s 有声音」，代价是「真答案晚 1-1.9s」。
这是**排队决策**，不是链路慢——所以「三腿都流式」再怎么优化也动不了它。

### 42.4 更该担心的不是慢，是**没回答**（第一发实测就撞到）

第一次真测（`call-2526aad3`）是病态通话，4/4 轮都没真答：

```
AGENT_METRICS eou delay=4449ms transcription=4448ms      ← eou 撞到全库最大
[watchdog] no assistant audio 4s after commit -> force-interrupt + ack
[agent] llm prefix prewarm skipped: APITimeoutError('Request timed out.')
```

账本里 4 轮回复**全是同一句**「唔好意思，系統頭先慢咗少少，我即刻幫你查。」——
那是 **watchdog 的道歉句**，不是答案。全库 `no assistant audio 4s after commit`
**共 85 次**（分布多通：5/4/4/3/3/3…）。eou 全库 n=2345：**p50 594ms / p90 1037ms / max 4449ms**。

即：**eou 尾巴一抖（4.4s）就把 LLM 挤过 4s 线，客户拿到的是一句道歉**。
这比「慢 1 秒」严重得多，且它是**功能性缺陷**不是体感问题。

### 42.5 测量口径的诚实边界

- 探针的 t0 = 推流循环结束时刻，而虚拟麦克风里的音频**还在往外放**，agent 的 VAD
  听到的「停嘴」比 t0 晚一个尾缓冲——**外部读数系统性偏高**（约几百 ms）。
  故 42.2 的绝对值是**上界**，轮间相对比较仍有效。
- `turns` 账本推的 `started_ms = 落库时刻 − TTFT` 只是**近似**：由它算出的
  「客户停嘴→生成起点 平均 4951ms」与日志侧 eou p50 594ms **对不上**，
  故 **§41 之前引用的 4951ms 不作为结论**，以日志打点 + 外部探针为准。

### 42.6 下一步的杠杆（按性价比，待讨论）

1. **别让回复等垫话**（收益最大、最自伤）：垫话现在是「买到 1.5s 有声、赔上 1-1.9s 真答案」。
   要么把垫话压短（1.0-1.5s 一档），要么**只在回复没就绪时才垫、就绪即放**
   （现在的 `hold_if_playing` 是无条件等到垫话播完）。
2. **eou 尾巴**（p90 1.0s、max 4.4s）：A 线是否开子句级提交（`QWEN3_ASR_CLAUSE_COMMIT`
   目前只给 B 线）——它能把成轮提前到说话中，但会改 A 线轮次语义，是延迟与准确率的直接交换。
3. **watchdog 那条路**：85 次「道歉代替回答」应优先于延迟优化处理——它已经越过了「体感」进入「没干活」。

---

## 43. 垫音的两个问题（2026-09-21）：**时长**自伤 + **内容**随机（用户定调）

用户定调：*「我们本身的目标是用户说完话后 1.5s 内开始听到 TTS 第一声，所以垫音；
但垫音太教条/太随机，**应该根据用户回复出对应的垫音**，让整体对话更自然。」*

### 43.1 先说 1.5s 这个目标的真实可达性（把预算拆到「可播」那一层）

§42 的三腿预算是 **eou 0.75 + LLM 首 token 0.7 + TTS 0.04 ≈ 1.5s**，但那是
「首 token」不是「可播音频」。TTS 要等**第一句生成完**才能念（tps 20-24，一句粤语
20-30 token ≈ 1.0s）。所以：

**回复首段可播音频 ≈ eou 0.75 + 首 token 0.7 + 首句生成 1.0 + TTS 0.04 ≈ 2.5s。**

即：**1.5s 对「垫音」这一路是可达的（实测首声 1.0-1.5s），对「真回复」这一路
按当前架构不可达**——除非把第一句做得更短（prompt 层可做：要求首句 ≤8 字），
或把 eou 压掉（子句级提交）。这条把「1.5s 目标」翻译成了两个具体杠杆。

### 43.2 时长：`hold_if_playing()` 自己掐自己 1.3s（已修）

`hold_if_playing()` 把回复首帧扣到「垫话播完 + gap」——**垫话多长，回复就被推迟多久**。
实测第 5 轮：回复音频 ~2.8s 就绪，却等垫话播完 3.8s + gap 才出声（`tts ttfb` 被撑到
1398ms）。而垫话是从「短档 1.0-1.5s / 长档 1.7-2.3s」里**随机**挑的，不看回复要多久。

修法：`BOK_FILLER_MAX_DUR_S`（默认 1.2s）——`_pick` 先按时长筛池再走去重，
只挑盖得住就够的短档；池里没有合规条目就保留整池（**绝不饿死垫话**，静音比长垫话更差）。
`0`=关回旧行为。踩到的坑：时长筛若放在 recent 去重**之后**，短档进去重窗后长档会漏回来
（4 条池连挑两次即复现，上限形同虚设）——已改为**先筛池**，测试钉住。

### 43.3 内容：79% 的垫音是「随机通用句」，与客户那句话无关（未修，需定方案）

垫音有两个来源：

| 来源 | 是什么 | 现状 |
|---|---|---|
| CP 罐头条目（`/api/fillers`，`FillerIndex` 按**客户原话**做词面匹配） | 24 条（每语言 8 条：`*-def/min/emp/ack/check/comp`），`hit_count` 多为 0 | **命中率 21%**（日志 82 hit / 308 miss） |
| 资产池（`assets/fillers`，5 场景类 × 20 条/语言） | 通用应承句，**内容不引用客户说了什么** | 匹配 miss 时**类内随机挑** |

真实通话的对应关系（真库，客户原话 → 垫音原话）：

| 客户说 | 垫音回 |
|---|---|
| 你哋系边间公司？ | 好嘅，你等一等，我好快覆你。 |
| 几时处理得好？ | 好嘅，你稍等陣，我而家就幫你睇下。 |
| 可以点样赔？ | 好嘅，你等陣，我幫你查下先。 |
| 唔系咁讲。（纠正） | 好嘅，你等一等，我好快覆你。 |

问身份/问时间/问赔偿/纠正，听到的全是同一类「等等我查」。**这就是用户说的「随机 + 教条」。**

**为什么是随机**：`FillerIndex` 是**词面匹配**（客户原话 vs 罐头条目文本），24 条太薄 →
79% miss → 落到资产池的**类内随机**。所以「对应」这条路机制**已经有了**，
缺的是**条目**（以及条目文本要按客户真实说法写，否则词面匹配不上）。

**出现最多、目前只能吃随机垫音的客户话**（真库；注意此榜被探针语料污染——
「你好，我想了解一下你。」「对的，我就是本人有话请讲。」是探针台词）：
投诉类（「我要投诉件货延误」11+5+5 次）、问赔偿（「可以点样赔？」6）、
问进度（「几时处理得好？」）、报单号（「我个单号系三七七八九零」）、
身份质疑（「你哋系边间公司？」）、纠正（「唔系咁讲。」）。

### 43.4 三条可选方案（按性价比，待定）

1. **补罐头垫话条目**（最小改动、立刻见效）：按上面那几类**用客户真实说法**写条目
   （词面匹配吃的是说法，不是语义），每条配预合成音频。命中率 21% → 目标是覆盖
   头部几类。注意：条目是**运营数据**（CP `/api/fillers`），不是代码。
2. **让垫音「带话题」**：把通用句换成引用客户关注点的短句
   （问赔偿 → 「賠償我幫你查緊，你等陣」；问进度 → 「我幫你睇下幾時處理好」）。
   这是**改资产池文案 + 重跑 `gen_filler_assets.py`**（固定音色预生成），仍是离线资产。
3. **能秒答的就不用垫**：客户问句若命中 QA 罐头（实测检索快路 ~50ms），
   直接播答案，垫音根本不需要——这是「对应」的极端形态（真的回答他）。
   现成的钩子在 `qa_gate`，问题是词条覆盖。
