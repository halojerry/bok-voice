...
> **修订（2026-09-14 拍板，取代下表四角色）**：角色收敛为三层——
> `root`（云端平台方 = 原 `super_admin` 职责）/ `admin`（本地主管：**静默旁听员工通话**
> + 暂停/掐断/接管/挂断，旁听与操作全程审计留痕；同承接原 `org_admin` 的本 org 管理面）/
> `user`（话务员：对象只读、**账号级问答库与话术可编辑**且 **每个话务员可有自己的话术**
> （per-user 归属）、通话工作台、账号级名册、**同传（B 线）与外呼战役**）。
> 数据边界 = `org → account（业务分组）→ user`：话务员只见本账号 + 本人资源，主管见整 org。
> 实施拆入路线 B（身份与角色层）；密码 hash 改用 stdlib `hashlib.scrypt`（免新增依赖），
> JWT 复用现成 PyJWT。本文下表保留为历史决策记录。
