...
- **ASR 热词/context 软偏置**：Qwen3-ASR 官方 customizable context（system message 词汇表「Vocabulary: …」，与 language 强制可叠加，mlx 层 `generate(system_prompt=)`）——治领域词误听（「單號」聽成「打啊」类，e2e_edge_cases E2 实证）。来源两层：行业静态词（`agent.py` `_ASR_HOTWORDS` 三语各一套，话术域高频词）+ 对象文字字段（`courier`/`contact_channel` 专名类）；**数字串唔进**（biasing 幻听数字风险，下游 known-number 过滤兜底）；总长护栏 ≤120 字符（partial 每 ~700ms 重解码，prefill 变贵）。装配时 `asr_hotword_context(asr_pin_lang, object_card)` 一次性组装，`Qwen3ASRSTT(hotword_context=)` 随 `/api/start` 下发、sidecar session 级透传每次解码。两级 kill-switch：`BOK_ASR_HOTWORDS=0`（agent 唔下发）/ `QWEN3_ASR_CONTEXT=0`（sidecar 唔透传）。改词表只动 `_ASR_HOTWORDS`；话术级定制（templates.hotwords 字段）二期。
