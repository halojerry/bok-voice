...
- **并发/边界回归资产（2026-09-07 QA）**：`scripts/e2e_barge_in.py`（打断：AI 播报中插话→断言停声+恢复不哑火）、`scripts/e2e_edge_cases.py`（静音/超短音频不毒化链路）、`scripts/load_cp_concurrency.py`（CP 独立 :8001+临时 DB 并发压测，含 turns 并发 30/30 回归——turn_id 竞态丢数据防复发）`scripts/e2e_interpret.py`（B 线同传双向 E2E：fwd/rev 双向译文音轨 ASR 回读断言目标语 + turns 原文/译文分行落库 + 启停×3 + 长流，2026-09-08 首跑 5/5）、`scripts/load_audio_concurrency.py`（N 路真实通话并发，`LOAD_ROADS/TURNS` 可调；单机上限参照 prompt-cache 4-6 路、TTS sidecar 全局锁串行）。改动 turns/audit/并发相关代码后必跑前三个。
