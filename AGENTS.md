...
- **ASR 热词/context 软偏置**：Qwen3-ASR 官方 customizable context（system message 词汇表「Vocabulary: …」，与 language 强制可叠加，mlx 层 `generate(system_prompt=)`）——治领域词误听。**实证（2026-09-07）**：e2e_edge_cases E2「我個單號係三七七八九零」前导误听（main/PR#31 三轮 ctx=False，「單號」聽成「打啊」）开启后 ctx=True 修复、7/7 全绿、真实通话轮零词表污染；context prefill 开销实测可忽略（真实长语音 A/B 无差），但**极低内容音频直打 sidecar 会把词表当转写抄出来**（纯静音探针稳定复现）——生产路径靠 VAD 门控+语音内容天然规避，留意短应承场景回归。来源两层：行业静态词（`agent.py` `_ASR_HOTWORDS` 三语各一套，话术域高频词）+ 对象文字字段（`courier`/`contact_channel` 专名类）；**数字串唔进**（biasing 幻听数字风险，下游 known-number 过滤兜底）；总长护栏 ≤120 字符。装配时 `asr_hotword_context(asr_pin_lang, object_card)` 一次性组装，`Qwen3ASRSTT(hotword_context=)` 随 `/api/start` 下发、sidecar session 级透传每次解码。两级 kill-switch：`BOK_ASR_HOTWORDS=0`（agent 唔下发）/ `QWEN3_ASR_CONTEXT=0`（sidecar 唔透传）。改词表只动 `_ASR_HOTWORDS`；话术级定制（templates.hotwords 字段）二期。
