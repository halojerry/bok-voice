...
与 ASR 语义自纠的互补关系(2026-09-22 真栈实弹):TTS 渲染「陪」(音系错字
形态)、ASR 转写自纠成「赔」——主 LLM 语料偏置会把常见错写拉回正字,即字面
层已隐性吃到部分自纠红利、音系层覆盖的是**自纠失败形态**(碎裂/生僻错字),
两层互补不冗余;同时 QA 挖词(tts-mine/gap-adopt)不能只看转写字面——真实
音系错字分布被自纠系统性低估。
