...
YUE_CANNED_VOICE = "Cantonese_crisp_news_anchor_vv2"
...
TRIGGER = "你要怎么陪我呢"          # 陪→赔 同音(yue 音系 0.838 / zh miss)
TRIGGER_LITERAL = "要点样赔"        # 词条原文(yue 字面 1.0)

LEG_TEXTS: dict[str, list[tuple[str, str]]] = {
    "hit":     [("warmup", WARMUP_YUE), ("trigger", TRIGGER)],
    "literal": [("warmup", WARMUP_YUE), ("trigger", TRIGGER_LITERAL)],
    "zh":      [("warmup", WARMUP_ZH), ("trigger", TRIGGER)],
    "kill":    [("warmup", WARMUP_YUE), ("trigger", TRIGGER)],
}
...
def create_probe_template(lang: str) -> str:
    steps = PROBE_STEPS_YUE if lang == "cantonese" else PROBE_STEPS_ZH
...
def log_windows(marks: list[int]) -> list[list[str]]:
    if not erc.LOG_PATH.exists():
        return [[] for _ in marks]
    data = erc.LOG_PATH.read_text(errors="replace").encode()
    wins: list[list[str]] = []
    for i, end in enumerate(marks):
        start = marks[i - 1] if i else 0
        chunk = data[max(0, min(start, len(data))): max(0, min(end, len(data)))]
        wins.append(chunk.decode(errors="replace").splitlines())
    return wins
...
async def run_leg(*, leg: str, lang: str, voice: str, keep_template: bool) -> dict:
    ...
    try:
        ...
        # 窗口 0 = 0→起始偏移 = 整个历史日志(混入他通话行,首跑实证污染 absence
        # 判据)——丢弃;轮 k 的行落在 marks[k]→marks[k+1](即 windows[k+1])。
        round_windows = windows[1:]
        names = [n for n, _ in rounds]
        win_by_name = {name: round_windows[i] for i, name in enumerate(names) if i < len(round_windows)}
        all_lines = [ln for w in round_windows for ln in w]
...
async def main() -> int:
    ...
    voice = args.persona_voice or (
        YUE_CANNED_VOICE if lang == "cantonese" else erc.SCENARIOS["zh"]["persona_voice"]
    )
