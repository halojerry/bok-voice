...
运行：.venv312/bin/python scripts/measure_latency.py   （需 sidecar 8787/8788 + Ollama）
"""
...
def measure_llm(client: httpx.Client) -> None:
    ...
    with client.stream(
        ...
        json={
            ...
            "stream": True, "think": False, "options": {"num_predict": 160},
        },
    ) as resp:
...
def measure_translate(client: httpx.Client) -> None:
    ...
    r = client.post(
        ...
        json={
            ...
            "stream": False, "think": False, "options": {"num_predict": 160},
        },
    ).json()
    print(f"翻译(一句)             : {ms(t0)}   out={r.get('message',{}).get('content','')[:30]!r}")


def measure_tts(client: httpx.Client, text: str, lang: str, label: str) -> None:
    t0 = time.perf_counter()
    r = client.post(
        f"{TTS}/v1/audio/speech", timeout=180,
        json={"input": text, "language": lang, "sample_rate": 24000},
    )
    dur = len(r.content) / 2 / 24000
    print(f"TTS 合成({label})       : {ms(t0)}  ({dur:.1f}s 音频)")
