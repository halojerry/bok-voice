...
def duel_verdicts() -> dict:
    """优先级对决腿(Phase 3.1,2026-09-18):同一问法三档胜者对照,离线零栈。

    legacy   = 两条目均无 priority 键(=线上现状:纯分数+插入序,老条目胜)
    priority = pinned 条目 priority 1 压过 old 条目 10(阈值过关者中小者先)
    kill     = BOK_QA_PRIORITY=0 回纯分数档(与 legacy 同胜者)
    主判据:legacy=old / priority=pinned / kill=old 三断言全过 exit 0。
    """
    from agent_runtime.qa_gate import QaIndex

    def _e(qid: str, a: str, **kw):
        return {"id": qid, "question_text": DUEL_QUERY, "answer_text": a,
                "lang": "zh", "scope": "global", **kw}

    def _winner(rows: list[dict]) -> str:
        hit, _ = QaIndex(rows).match(DUEL_QUERY)
        return str(hit.get("id")) if hit else ""

    saved = os.environ.get("BOK_QA_PRIORITY")
    try:
        os.environ.pop("BOK_QA_PRIORITY", None)
        legacy = _winner([_e("old", "a-old"), _e("pinned", "a-pin")])
        priority = _winner([_e("old", "a-old", priority=10), _e("pinned", "a-pin", priority=1)])
        os.environ["BOK_QA_PRIORITY"] = "0"
        kill = _winner([_e("old", "a-old", priority=10), _e("pinned", "a-pin", priority=1)])
    finally:
        if saved is None:
            os.environ.pop("BOK_QA_PRIORITY", None)
        else:
