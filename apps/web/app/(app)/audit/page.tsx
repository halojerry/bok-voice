    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-(--card-border) bg-(--card) p-4">
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          动作
          <select className="rounded-lg border border-(--card-border) bg-transparent px-3 py-2 text-sm" value={action} onChange={(e) => setAction(e.target.value)}>
            {ACTIONS.map((a) => <option key={a} value={a}>{a || "全部"}</option>)}
          </select>
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          账号
          <input className="rounded-lg border border-(--card-border) bg-transparent px-3 py-2 text-sm" placeholder="acc-001" value={accountId} onChange={(e) => setAccountId(e.target.value)} />
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          通话
      ...
      {loading ? <LoadingState label="加载审计记录…" /> : error ? <ErrorState message={error} /> : rows.length === 0 ? <EmptyState label="暂无审计记录" /> : (
        <div className="overflow-x-auto rounded-xl border border-(--card-border)">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-(--card-border) text-xs text-muted-foreground">
              <tr>
