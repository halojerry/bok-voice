        ...
        <section className="space-y-6">
          ...
          <div className="card">
            ...
            <ul className="mt-2 space-y-2 text-xs leading-relaxed muted">
              <li>· 导入内容会整篇落盘为 Markdown：<span className="text-(--foreground)">账户数据目录/vault/accounts/{`{账号}`}/knowledge/…</span>，同时建立检索引擎索引。</li>
              <li>· 每场通话 Agent 会按客户问题自动检索该账号知识，把命中的片段（约 3–5 条）随 system 指令注入大模型，作为回答依据。</li>
              <li>· 在这里「检索」可实时看到哪些知识会被命中；删除某条会同步清理 vault 文件与索引。</li>
              <li>· 挂断结算后，通话转写与结算文档也会落盘到 <span className="text-(--foreground)">…/objects/{`{对象}`}/calls/{`{通话}`}/transcript.md 与 settlement.md</span>。</li>
