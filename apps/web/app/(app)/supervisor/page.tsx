      ...
      <section className="card">
        ...
        {calls.length === 0 ? (
          ...
          <div className="space-y-3">
            {calls.map((c) => (
              <div key={String(c.call_id ?? c.id)} className="rounded-lg bg-white/5 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button
                    className="rounded border border-red-500/30 bg-red-500/10 px-3 h-7 text-xs font-semibold text-red-400 transition hover:bg-red-500/20"
                    onClick={() => {
                      if (window.confirm("确认挂断并关闭该通话？"))
                        send(api.hangup, String(c.id ?? c.call_id));
                    }}
                  >
                    挂断
                  </button>
